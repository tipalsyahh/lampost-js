document.addEventListener('DOMContentLoaded', async () => {
  ...
  const container = document.querySelector('.data-microsite-banklampung');
  if (!container) return;
  ...
  async function loadPosts() {
    try {
      const api =
        'https://lampost.co/microweb/banklampung/wp-json/wp/v2/posts' +
      ...
      posts.forEach(post => {
        ...
        const BASE = 'https://lampost.co/microweb/banklampung';
