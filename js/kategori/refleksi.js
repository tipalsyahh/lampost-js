document.addEventListener('DOMContentLoaded', () => {
  ...
  // =====================
  // GET KATEGORI REFLEKSI SEKALI
  // =====================
  (async () => {
    try {
      const res = await fetch(
        'https://lampost.co/wp-json/wp/v2/categories?slug=refleksi'
      );
  ...
  async function loadPosts() {
    ...
    try {
      ...
      posts.forEach(post => {
        ...
        htmlArr.push(`
          <a href="../halaman.html?refleksi/${slug}" class="item-info" id="${id}">
            <img src="image/ai.jpg" class="img-microweb" loading="lazy">
            <div class="berita-microweb">
              <p class="judul">${judul}</p>
              <p class="kategori">Refleksi</p>
        ...
        // GAMBAR + EDITOR ASYNC (NON BLOCKING)
        (async () => {
