  ...
  (async () => {
    try {
      const res = await fetch(
        'https://lampost.co/wp-json/wp/v2/categories?slug=podcast'
      );
      const data = await res.json();
      kategoriId = data?.[0]?.id;
      loadPosts();
    } catch {
      container.innerHTML = '<p>Kategori potcast tidak tersedia</p>';
    }
  })();
  ...
  async function getMedia(mediaId) {
    ...
    try {
      ...
      return (mediaCache[mediaId] =
        ...
        data.source_url ||
        '/lampost.co/image/ai.jpeg'
      );
    } catch {
      return '/lampost.co/image/ai.jpeg';
    }
  }
