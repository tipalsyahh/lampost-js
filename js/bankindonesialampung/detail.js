document.addEventListener('DOMContentLoaded', async () => {
  ...
  // 🔧 FIX URL ?headline/search (TANPA ubah logic lain)
  if (slug === 'search') {
    window.location.href =
      `../search.html?q=${encodeURIComponent(kategoriSlug)}`;
    return;
  }
  ...
  try {
    const api =
      `https://lampost.co/microweb/bankindonesialampung/wp-json/wp/v2/posts?slug=${slug}&_embed`;
    ...
    isi.querySelectorAll('a[href]').forEach(link => {
      ...
      try {
        ...
        if (slugBerita) {
          link.href =
            `berita.bankindonesialampung.html?${kategoriSlug}/${slugBerita}`;
          link.target = '_self';
        } else {
